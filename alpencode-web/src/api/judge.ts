...
export function runCode(data: RunCodeParams) {
  return request.post<any, R<RunCodeResult[]>>('/ac/judge/run', data);
}

/** Submit — 提交判题 */
export function submitCode(data: SubmitCodeParams) {
  return request.post<any, R<{ submitId: number }>>('/ac/judge/submit', data);
}

/** 查询提交结果（轮询用） */
export function getSubmitResult(submitId: number) {
  return request.get<any, R<import('./types').Submit>>(`/ac/submit/info/${submitId}`);
}
